...
mod instructions {
    ...
    impl Instruction {
        ...
        pub fn execute(&self, stacks: &mut Vec<Vec<char>>) {
            let items = stacks.get_mut(self.from)
                .map(|stack| stack.split_off(stack.len() - self.amount_to_move))
                .expect("Index out of bounds.");

            stacks.get_mut(self.to)
                .map(|stack| stack.extend(items.into_iter()))
                .expect("Index out of bounds.");
        }
    }
}
